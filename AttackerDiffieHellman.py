...
class AttackerDiffieHellman(object):
    def __init__(self, alice_file, bob_file, generator=2, group=17, keyLength=1024):
        default_generator = 2
    ...
    def publishKey(self):
        with open(self.alice_file, "a") as file:
            file.write(f"Bob-key:{str(self.publicKey)}\n")
        with open(self.bob_file, "a") as file:
            file.write(f"Alice-key:{str(self.publicKey)}\n")
    def readAliceKey(self):
        while True:
            try:
                ...
                for line in reversed(lines):
                    ...
                    if "Bob" not in formattedLine and formattedLine != str(self.publicKey) and "|" not in formattedLine:
                        front , ciphertext_str = formattedLine.split(":")
    ...
    def readBobKey(self):
        while True:
            try:
                ...
                for line in reversed(lines):
                    formattedLine = line.replace("\n","")
                    if "Alice" not in formattedLine and formattedLine != str(self.publicKey) and "|" not in formattedLine:
                        front , ciphertext_str = formattedLine.split(":")
                        return int(ciphertext_str)
                    iteration_count += 1
                    if iteration_count >= 2:
